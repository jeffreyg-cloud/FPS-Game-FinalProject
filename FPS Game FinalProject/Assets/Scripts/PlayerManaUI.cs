...
public class PlayerManaUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (manaFill != null) fullWidth = manaFill.sizeDelta.x;
        else Debug.LogWarning("PlayerManaUI: Mana Fill has not been assigned.");
    ...
    {
        if (currentMana >= maxMana || manaRegenPerSecond <= 0f) return;
    ...
    private void AnimateManaBar()
    {
        ...
        float newWidth = Mathf.MoveTowards(manaFill.sizeDelta.x, targetWidth, changeSpeed * Time.deltaTime);
        manaFill.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, newWidth);
    }
    ...
    private void UpdateManaUI(bool updateImmediately)
    {
        ...
        if (updateImmediately && manaFill != null)
            manaFill.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, targetWidth);
        UpdateManaText();
    }
